...
#Anything not listed in here will geta  month to live
TWOWEEKS = ('tsuki','devtools','ftwotools')
...
#arg[0] = cacheTree; arg[1] = rootNode; arg[2] = expDate; arg[3] = today; 
    ...
    if names[i] in ("CHANGELOGS","pe040201","moveout","fb031223","dpb031230_64-1","newformat"): #Skip specific dirs
      del names[i]
...
readmePath = "/mvista/dev_area/README_keep" #where the listing of files to be kept is
...
for line in keepFile.readlines():
  ...
  if extracted != None:
    ...
    tempData.expDate  = string.atoi(extracted.group(2)) #we want a number not a string
    tempData.emailAddr= extracted.group(3)
    if rootNode == None:
