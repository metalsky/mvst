...
import smtplib, re, os, sys, string
from email.MIMEText import MIMEText

smtpServer = 'mail.mvista.com'
sender = 'build@mvista.com'
destinationEmail ="rell@mvista.com"

DEBUG = 1

def warningMessage():
  os.system('/home/build/build_scripts/utils/findOldDirs.py > /home/build/build_scripts/utils/trashed_dirs')
  try:
  ...
  for removalDir in removal_list:
	...
	removeLocation = '/mvista/dev_area/%s/REMOVE/cdimages/'%(product) 
      ...
      if DEBUG:
        ...
        if removeLocation != None:
          os.system("mkdir -p %s"%removeLocation)
          os.system("mv %s  %s"%(string.strip(removalDir), removeLocation))
      parsed = None
...
def deleteRemovalDirs():
  os.system('rm -rf `find | grep REMOVE`')
