...
exec(open(builddir + '/proadkdata.dat'))
...
cddir = '/mvista/dev_area/pro/cdimages/adk/' + buildtag
print 'cddir = ' + cddir
os.system('mkdir -p ' + cddir)
# make a temporary cd directory to build cdimages locally, then copy to dev_area
...
os.chdir(builddir)
logdir = '/mvista/dev_area/pro/adk/' + buildtag
...
#------------------------------------------------------------------------------------------
  ...
  for target in os.listdir(installrpmdir + '/target'):
    ...
    for vd in vdirs:
      ...
      # copy the x86_586 cross/target install files into each arch cross/target install area
      for target in os.listdir(vd):
        os.system('cp -a ' + vd + '/x86_586/* ' + vd + '/' + target)
...
if mktargetcd:
  for cdtype in cdnames:
    ...
    for cd in all_targets:
      ...
      if cd != 'x86_586':
        mkisocmd = mkisocmd + 'x86_586/' + cdtype + '/=' + cpdir + '/x86_586/' + cdtype + '/ '
        #mkisocmd = mkisocmd + 'x86_586/target/=' + cpdir + '/x86_586/target/ '
...
if mkcdbom:
  ...
  sys.__stdout__.flush()
  os.system('./bommaker.py %s %s' % ('pro','adk/'+ buildtag))
  sys.__stdout__.flush()
